...
| S3 | 上游许可是 **Apache-2.0**，而 README「许可」小节此前只写 MIT | 补一句"本仓库是 Apache-2.0 项目的 WPF 移植版，保留上游版权与署名"。**是否随附 Apache-2.0 许可证副本（`LICENSE-APACHE`）未擅自添加** —— 属许可文书，写成 TASKS 的待决项交还用户 |
