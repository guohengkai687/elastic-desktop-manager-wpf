...
审查范围：`0140fde..工作区`（用户指令："修改作者 lxwise 改为 guohengkai，对应的 github 的 url 也要做修正"）。
审查方式：**主代理自查 + 静态守卫**（同前几轮的流程说明：审查独立性弱于有独立审查子代理的轮次，如实记录）。
审查轴：标准轴（署名事实、文案唯一来源、守卫、零警告）+ 规格轴（用户指令原文）。

### 需求达成

- 「关于」窗口的作者字段（`app.author`，zh/en）→ `guohengkai`；GitHub 按钮（原来写死 `github.com/lxwise/...`）
  → 本仓库 URL，并抽成 `AboutWindow.RepoUrl` 常量，不再是散落的字面量。**达成。**
- 「对应的 url 也要修正」的**边界**：只改**本移植版自己的**链接。上游 `lxwise/elastic-desktop-manager`
  的链接出现在 `README`（移植来源说明、「许可」小节）与归档审查记录中，属 **Apache-2.0 的署名要求**，
  保留不动 —— 删掉会让移植版看起来是原创，既是许可问题也是事实问题。

### 本轮自查发现并处置的问题

| ID | 问题 | 处置 |
|---|---|---|
| S1 | 「关于」窗口的作者名与 GitHub 按钮沿用了原版的**值**（移植时抄了 code-behind 结构与取值） | 词条改 `guohengkai`；新增 `RepoUrl` 常量；`about.desc` 中英补上游署名，使"本仓库作者"与"上游作者"同时可见 |
| S2 | 这类"值是别人家的 URL/名字"的缺陷**没有任何门禁能抓**：编译不报错，单测与既有 29 项守卫（只看绑定契约、资源、文案**来源**）全绿 | 新增守卫规则 + 自检 + 负向验证。规则**只扫 `src/**/*.cs`**：README/docs 的上游署名链接是许可要求，扫进去会逼人删署名 |
| S3 | 上游许可是 **Apache-2.0**，而 README「许可」小节此前只写 MIT | ① 先补一句"本仓库是 Apache-2.0 项目的 WPF 移植版，保留上游版权与署名"，并把"是否随附许可证副本"作为待决项交还用户；② **用户随后决定加** → 新增 `LICENSE-APACHE`（apache.org 官方全文，md5 校验一致）+ `NOTICE`（上游署名 + Apache-2.0 §4(b)/4(c) 要求的"已修改"声明）；README「许可」小节改为 MIT / Apache-2.0 **双许可表** + 使用提示（并注明非法律意见） |

### 验证证据

- 构建 **0 警告 0 错误**；Core 单测 **105 → 106/106**（新增 1 条：署名串在 zh/en 下的实际取值 + `about.desc` 仍保留上游署名）；
  静态守卫 **29 → 31/31**（1 条新规则 + 1 条新自检，自检总数 10 → 12）。
- **负向验证 2 条**（改坏 → 确认只有预期那一条失败且信息精准 → 还原 → 复跑全绿）：
  ① 把 `RepoUrl` 注入回 `https://github.com/lxwise/elastic-desktop-manager` → 守卫只报这一条，
  信息精确到文件：`AboutWindow.xaml.cs: github.com/lxwise/elastic-desktop-manager`（`通过 30，失败 1`）；
  ② 把 `app.author` 注入回 `lxwise` → 单测只报这一条：`zh author: expected <作者：guohengkai> but got <作者：lxwise>`（`通过 105，失败 1`）。
- 新规则自检的 5 个方向里含一条**反证**（把 `own` 换成别的值后同一个 URL 必须报错），
  防止规则退化成"永远通过"。
- 许可文书核对：`LICENSE-APACHE` 的 md5 = `3b83ef96387f14655fc854ddc3c6bd57`（**与
  `https://www.apache.org/licenses/LICENSE-2.0.txt` 官方一致**，202 行 / 11358 字节）；与上游仓库分发的
  `LICENSE` 逐行 diff 仅差一个前导空行（同一份许可文本，未被人改过）；`LICENSE` / `LICENSE-APACHE` /
  `NOTICE` 三文件均在仓库根并被 README 引用。

### 关于「作者改名」的边界（记录判断依据）

用户指令"作者 lxwise 改为 guohengkai，对应的 github 的 url 也要做修正"在本仓库里对应**两类**不同的链接：
**本移植版自己的**（关于窗口的作者字段与 GitHub 按钮 —— 属于错误，已改）与**上游项目的署名**
（README 移植来源、「许可」小节、归档审查记录里的 `com/lxwise/...` —— 属于许可要求与历史事实，保留）。
判断依据是先读了上游 `LICENSE`（**Apache-2.0，不是 MIT**），确认署名不可省，再动手改。
本轮的许可文书补齐即由此而来。

### 明确无法在本机验证

- 「关于」窗口的实际渲染与按钮跳转（作者行文字、浏览器打开的地址）—— Linux 无 WPF，见 QA 第 37 条。
- 规则只覆盖 `src/**/*.cs`：`README`/`docs` 里如果哪天出现**本项目的**错误链接，本规则不管（有意为之，见 S2）。
